...
#   LMTUNE_DB              기본    data/db/lmtune.duckdb
...
LMTUNE_DB="${LMTUNE_DB:-data/db/lmtune.duckdb}"
...
lmtune sweep --profile-dir "$PROFILE_DIR" --endpoint "$ENDPOINT" --continue-on-error \
    2>&1 | tee "$OUT_DIR/sweep.log"

# 이번 sweep 의 run_id 들 수집 (모든 profile 에 대해 가장 최근 20건)
RUN_IDS=$(lmtune ls -n 20 | awk 'NR>2 {print $2}' | tail -n +1 || true)

# BASELINE 이 있으면 각 신규 run 과 diff 리포트 생성
if [[ -n "${BASELINE_RUN:-}" ]]; then
    while read -r rid; do
        [[ -z "$rid" || "$rid" == "run_id" ]] && continue
        lmtune compare "$BASELINE_RUN" "$rid" --threshold-pct "$REGRESSION_THRESHOLD" \
