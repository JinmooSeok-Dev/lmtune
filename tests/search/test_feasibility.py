...
def test_c5_ep_must_divide_experts_for_moe(constraints):
    ...
    p = _make_params(expert_parallel_size=3, tensor_parallel_size=8,
                     pipeline_parallel_size=1)
    env = Environment.b200_dual_node()
    rep = evaluate(p, environment=env, constraints=constraints,
                   model=by_name("Mixtral-8x22B"))
...
def test_is_feasible_convenience(constraints):
    env = Environment.b200_dual_node()
    p = _make_params()
    assert is_feasible(p, environment=env, constraints=constraints,
                       model_name="Llama-3.1-70B") is True
    p_bad = _make_params(decode_context_parallel_size=3, tensor_parallel_size=4)
    assert is_feasible(p_bad, environment=env, constraints=constraints,
                       model_name="Llama-3.1-70B") is False
...
def test_is_feasible_with_yaml_path(tmp_path):
    ...
    f.write_text(yaml.safe_dump({
        ...
        "feasibility_constraints": [
            {"id": "x", "rule": "tensor_parallel_size <= environment.npus_per_server",
             "message": "tp fits"},
        ],
    }))
    env = Environment.b200_single_node()
    assert is_feasible({"tensor_parallel_size": 4}, environment=env,
                       space_yaml_path=f) is True
    assert is_feasible({"tensor_parallel_size": 16}, environment=env,
                       space_yaml_path=f) is False
