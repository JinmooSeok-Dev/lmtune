...
DuckDB 는 한 파일에 단일 프로세스 writer 만 허용. S3 의 `Study.storage.suspend() / resume()` 은 자식 subprocess 가 bench run → DuckDB 쓰기를 해야 할 때 부모가 잠시 락을 풀어주는 shim. 분산 스케일에서 writer queue (S3 `writer_queue.py`) 로 대체 예정.
